...
5. **Scene 1 is the HOOK** — the most arresting, curiosity-triggering image of
   the whole video. Whatever the narration opens with, scene 1 must stop a
   thumb.
6. **Exaggerate the emotion** like a top animated short: the pose reads from
   across a room, not from a foot away.
7. **Vary shot size and angle HARD** between consecutive scenes — extreme
   close-up on the eyes, then wide, then top-down, then over-the-shoulder.
   Never two similar framings in a row.
...
start. Name what moves first, then how fast.

**The camera may move, and usually should** — a fast push-in, an orbit, a rise.
This is the one place v2 breaks with the old channel, which kept the camera
still and let only the frame change. AFFILIATE, whose shorts hold attention,
does the opposite and uses camera movement as its main engine.

Forbidden words in a video prompt: `subtle`, `slight`, `gentle`, `slowly`.
