...
#: Chỗ trống mà một khuôn chia cảnh PHẢI có thì `prompt.workbook` mới dùng nó
#: thay khuôn mặc định (`core.chia_canh.KHUON_MAC_DINH`). Thiếu một chỗ là AI
#: không nhận được phụ đề / dàn nhân vật / kế hoạch — nên thà bỏ khuôn khách
#: sửa còn hơn chạy với một lời nhắc cụt. Một danh sách, hai nơi đọc: ở đây
#: (ô Nâng cao kiểm trước khi gửi) và `prompt.workbook/run.py` (`_khuon_chia`).
CHO_TRONG_KHUON_CHIA = ("<<CAST_STYLE>>", "<<DIRECTOR_PLAN>>", "<<CONTEXT>>",
                        "<<SRT>>", "<<MAX_SEC>>", "<<KHUC_THU>>")


def khuon_chia_dung_duoc(khuon: str) -> bool:
    """Khuôn chia cảnh khách sửa có đủ chỗ trống để chạy không."""
    k = str(khuon or "")
